...
export function CollectionsLanding() {
  return (
    <PageShell solidHeader>
      <section className="relative bg-fog text-graphite">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-linear-to-b from-ink/10 to-transparent" aria-hidden="true" />
        <div className="mx-auto flex max-w-6xl flex-col items-center px-8 py-16 text-center md:px-20 md:py-24 lg:px-16 lg:py-32" data-reveal>
          <p className="type-eyebrow m-0 text-graphite/70">Đồng hành cùng phong cách của bạn</p>
          <h1 className="mt-3 text-3xl font-light tracking-[0.08em] uppercase md:text-4xl lg:text-5xl">Đồng hồ</h1>
          <p className="mt-4 max-w-2xl text-[15px] leading-relaxed text-graphite/85">
      ...
      {covers.map((cover, index) => (
        ...
        >
          ...
          <h2 className="mt-1 text-3xl font-light tracking-[0.08em] uppercase md:text-4xl lg:text-5xl">{cover.title}</h2>
          <div className="mt-10">
            <PillLink href={cover.href}>Xem thêm</PillLink>
          </div>
        </ParallaxCover>
      ))}
