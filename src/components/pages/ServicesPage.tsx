  ...
  return (
    <PageShell>
      <section className="relative h-[70svh] min-h-[520px]" aria-label="Dịch vụ chính hãng">
        ...
        <div className="rail relative z-[1] flex h-full flex-col justify-end pb-16 text-paper" data-reveal>
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-12 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((service, index) => (
            <article key={service.href} className="group/service flex flex-col" data-reveal style={{ "--reveal-delay": `${index * 80}ms` } as React.CSSProperties}>
              <Link href={service.href} className="relative block aspect-[4/5] overflow-hidden border border-line bg-tile" aria-label={service.title}>
              ...
              <p className="type-body mt-3 text-fg/70">{service.copy}</p>
