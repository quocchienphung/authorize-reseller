      ...
      <section className="rail pb-28" aria-label="Bảng giá">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] border-collapse text-[15px] font-light">
            <thead>
              <tr className="type-eyebrow border-b border-fg/25 text-left text-fg/55">
            ...
            <tbody className="divide-y divide-line">
              {rows.map((product) => (
                <tr key={product.slug} className="transition-colors hover:bg-fg/5">
                  <td className="py-4">
                    <Link href={routes.product(product.slug)} className="hover:opacity-60">
                      {product.name}
                    </Link>
                  </td>
                  <td className="py-4 tracking-[0.06em] text-fg/70">{product.sku}</td>
                  <td className="py-4 text-fg/70">{product.category}</td>
