...
export function ProductDetailPage({ product }: { product: Product }) {
  ...
  return (
    <PageShell solidHeader>
      ...
      <section className="rail grid gap-12 pt-8 pb-20 lg:grid-cols-[minmax(0,460px)_1fr] lg:items-center lg:gap-20 lg:pb-28">
        <div className="flex flex-col lg:order-1" data-reveal>
          ...
          <h1 className="m-0 mt-5">
            <span className="type-display block">Alexander Ferros</span>
            <em className="type-display-serif block">{product.sku}</em>
          </h1>
          <p className="mt-8 text-2xl font-light">{product.price}</p>
          <p className="type-body mt-6 max-w-[440px] text-paper/75">{product.description}</p>
      ...
      {/* Key facts */}
      {facts.length ? (
        <section className="rail border-y border-paper/15 py-10" aria-label="Thông số nổi bật">
          <dl className="m-0 grid grid-cols-2 gap-x-6 gap-y-8 md:grid-cols-3 lg:grid-cols-5" data-reveal>
            {facts.map((fact) => (
              <div key={fact.label}>
                <dt className="type-eyebrow text-paper/55">{fact.label}</dt>
      ...
      {variants.length > 1 ? (
        <section className="rail py-20 md:py-24" aria-label={`Các phiên bản ${reference}`}>
          <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between" data-reveal>
            <h2 className="m-0">
              <span className="type-display block">Dòng {reference}</span>
              <em className="type-display-serif block">{variants.length} phiên bản</em>
            </h2>
              ...
              return (
                <li key={variant.slug}>
                  ...
                  >
                    ...
                    <span className="block bg-ink px-2 py-2 text-center text-[11px] tracking-[0.08em] text-paper/80 uppercase">
                      {variant.sku}
                    </span>
                  </Link>
                </li>
      ...
      {/* Full specifications */}
      <section id="thong-so" className="rail border-t border-paper/15 py-20 md:py-24" aria-label="Thông số kỹ thuật">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,420px)_1fr] lg:gap-20">
          <div data-reveal>
            <h2 className="m-0">
              <span className="type-display block">Thông số</span>
              <em className="type-display-serif block">kỹ thuật</em>
            </h2>
            <p className="type-body mt-6 text-paper/70">
          ...
          <dl className="m-0 divide-y divide-paper/15 border-y border-paper/15" data-reveal="fade">
            {product.specifications.map(([label, value]) => (
              <div key={label} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] gap-6 py-4 text-[15px]">
                <dt className="font-light text-paper/60">{label}</dt>
